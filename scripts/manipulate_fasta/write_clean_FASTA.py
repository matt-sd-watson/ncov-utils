...
def main():
    parser = argparse.ArgumentParser(description='Write a cleaned multi-FASTA considering consensus record length '
        ...
        for record in fasta_sequences:
            try:
                ...
                if len(record) >= args.length:
                    count += 1
                    SeqIO.write(record, handle, "fasta")
                    print("WARNING: Adding record with length 0, completeness could not be calculated: {}".format(record.name))
