---
title: "SARS-CoV2 Genomic Surveillance Report"
...
date: "Period: `r format(Sys.time() - 7*24*3600, '%B %d, %Y')` to `r format(Sys.time(), '%B %d, %Y')`"
mainfont: Arial
fontsize: 11
link-citations: yes
urlcolor: purple
...
## Sequencing Summary, 2020
\
\

```{r, echo=F, message=F, warning=F, fig.width=13}
...
cat_time_plot <- ggplot(cat_over_time, aes(x = epi.wk.login, y = samples, fill = lineage.all.cat)) +
  ...
  theme(axis.text.x = element_text(angle = 90, size = 14),
        ...
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        legend.title = element_text(size = 14),
  legend.text = element_text(size = 12)) + labs(fill = "Category") + xlab("Epidemiological Week") + ylab("Sample Counts") + ylim(c(0, max(cat_over_time$samples) + 1000)) +
...
```{r, fig.width = 15, fig.height = 10, echo=F, warning=F, message=F}
...
ggplot(grouped_by_epi, aes(x = epi.wk.login, y = n, fill = lineage_id)) +
  geom_rect(aes(xmin = which(epiwk.list==end.epi.wk)-2.5,
             ...
             size = 4, vjust = 1, hjust = 0, color = "black") +
...
plot_1 <- ggplot(rolling_cases, aes(x = date, y = cumcases, color = lin_fac)) + geom_point() +
