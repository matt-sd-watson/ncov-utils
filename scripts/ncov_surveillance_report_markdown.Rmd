...
```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri("pho-smlogo-en-crop.png"),
alt = 'logo', style = 'position:absolute; top:0; left:0; padding:0;')
```
...
cat_time_plot <- ggplot(cat_over_time, aes(x = epi.wk.login, y = samples, fill = lineage.all.cat)) +
  ...
  legend.text = element_text(size = 12)) + labs(fill = "Category") + xlab("Epidemiological Week") + ylab("Sample Counts")
...
for (i in voc) {
  ...
  group_by(epi.wk.login, HEALTH_UNIT_REPORTABLE) %>% summarise(Samples = n())
