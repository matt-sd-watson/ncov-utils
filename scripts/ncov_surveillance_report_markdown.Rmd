...
for (i in unique(merged$lineage_all)) {
  ...
  plot <- plot_ly(subset, labels = ~ region_counts, values = ~percent, type = 'pie',
    ...
    layout(title = paste("Health Unit Distribution of ", title_name, sep=""),
           ...
           showlegend = T)
  plot_animated <- ggplotly(plot) %>% onRender("function(el,x){el.on('plotly_legendclick', function(){ return false; })}")
...
high_daily <- ggplot(highest_daily_ontario,
  ...
  ggtitle(paste("Pango Lineages w/ highest daily rates, past month, non-VoC, as of ", format(Sys.Date(), "%b-%d-%Y"), sep="")) + xlab("Logging Date") + ylab("Daily Case Number Increase")
...
tree_paths
```
# Phylogenetic Analysis of Pango Lineage VoCs
...
```{r, echo=FALSE, warning=FALSE, fig.height=13, fig.width=12}
for (i in tree_paths) {
  tree <- read.tree(i)
