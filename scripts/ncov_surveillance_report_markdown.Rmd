---
title: "SARS-CoV-2 Genomic Surveillance Report"
# <img src=\"pho-smlogo-en-removebg-preview.png\" style=\"float: left; width 50;\"/> "
...
```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri("pho-smlogo-en-crop-small.png"),
alt = 'logo', style = 'position:absolute; top:0; left:0; padding:0.0001;')
```
...
# Background
The continued monitoring of global SARS-CoV-2 genomic data has allowed for the identification of small changes in the genome as it spreads through populations. These random changes or mutations occur as a virus replicates imperfectly inside a host’s cells, and commonly occur as a virus evolves over time. The accumulation of mutations can result in a new lineage of the virus, which is a common occurrence. As SARS-CoV-2 has spread, globally distinct lineages have arisen, which are often visualized as phylogenetic (evolutionary) trees. 
In December 2020, Public Health England identified a new variant in the UK with concerning mutations, largely in the S gene which encodes the spike protein. This protein is responsible for recognition, binding and entry of the virus to the host cell. These mutations have been linked to increased transmissibility, disease severity and immune evasion. Because these changes have clinical and public health significance, viruses that contain these mutations are called a variant of concern (VOC). Currently, there are three globally defined VOCs; B.1.1.7 (first identified in the UK), B.1.351 (first identified in South Africa) and P.1 (first identified in Brazil). There are a number of other SARS-CoV-2 variants which contain mutations that may have biological and public health implications and warrant monitoring. These are termed Variants of Interest (VOI), and the prevalence of these may vary between regions.
Data included in this report are from Public Health Ontario’s genomics program in which SARS-CoV-2 sequences that pass basic quality control metrics are included. These sequences represent a combination of targeted and surveillance sampling.
\

## Aims
Whole genome sequencing has contributed significantly to understanding the origin of SARS-CoV-2 and its global spread. Continued sequencing of new infections is key to enhanced surveillance and monitoring as part of an effective pandemic response. Development of a large-scale sequencing program requires cutting-edge laboratory technology in addition to scientific and technical experts. Public Health Ontario (PHO) is well-equipped with these resources and together with access to population-level data, PHO is well-positioned to contribute significantly to SARS-CoV-2 WGS efforts.
There are several areas for which SARS-CoV-2 genome sequencing contributes to the COVID-19 pandemic response, allowing public health experts to:
\
\
  • Provide enhanced surveillance and monitoring for emerging variants
\
  • Investigate outbreaks
\
  • Contribute to continued improvements in diagnostic testing, vaccine development, and potential           treatments
\
...
qc_data <- read.table(paste("/home/mwatson/COVID-19/qc50/", qc50_loc, sep=""), header = T, sep = ',',
                      fill = TRUE, quote = "")
qc_data <- subset(qc_data, Processed_results == "Y")
# qc_data <- dplyr::distinct(qc_data)
...
total_lin_counts <- subset(qc_data_recent, lineage_all %in% voc) %>%
  arrange(date) %>%
  group_by(lineage_all) %>% dplyr::summarise(total = n())
...
meta.df <- qc_data %>%
            ...
            mutate(lineage.all.cat = ifelse(lineage_all == "B.1.1.7" | 
                                     ...
                                     ifelse(lineage_all == "P.2" | 
                                            ...
                                            lineage_all == "B.1.429" |
                                            lineage_all == "B.1.526" |
                                            lineage_all == "B.1.617.1" |
                                            lineage_all == "B.1.618" |
                                            lineage_all == "B.1.617.2" |
                                            lineage_all == "B.1.617.3",
                                            "VoI", "non-VoC/non-VoI"))) %>%
...
date_cat <- interval(ymd(Sys.Date() - 21), ymd(Sys.Date()))
meta.df$time_cat <- ifelse(meta.df$Login.Date %within% date_cat, "Last Three Weeks", "All Past Weeks")

time_cat_table <- as.data.frame(table(meta.df$time_cat))
...
num_lineages <- data.frame(Category = paste("Number of unique Pango lineages ",
                                            "(January 1, 2021-", format(Sys.time(), "%B %d %Y"),
                                            ")", sep=""),
...
n.lin.grp$Category <- ifelse(n.lin.grp$`Pango Lineage` %in% c("B.1.1.7",
                                                              "B.1.351", "P.1"), "VoC",
                             ifelse(n.lin.grp$`Pango Lineage` %in% c("P.2", "P.3", "B.1.525", "R.1", "B.1.1.318", "B.1.427", "B.1.429", "B.1.526",
              "B.1.617", "B.1.617.1", "B.1.617", "B.1.618", "B.1.617.2", "B.1.617.3"), "VoI", ""))
...
The number of genome sequences are summarized below and classified by the total number as of this report and the number of genomes sequenced in the most recent week by Public Health Ontario.
\
\
```{r, results='asis', echo=FALSE}
kable(as.data.frame(time_cat_table), caption = "Number of genome sequences by week")
```

## Sequencing Summary, 2020 and 2021

```{r, echo=F, message=F, warning=F, fig.width=16, fig.height=10}

## THIS IS CODE FOR SEQUENCING SUMMARY FOR BOTH 2020 and 2021
### SARAH SECTION 2 - Weekly Sequencing Summary ---- ###
# Generate a list of epi weeks in the dataset

epiwk.list <- meta.df %>%
                mutate(epi.wk.login.ord = epi.wk.login) %>%
                distinct(epi.wk.login, epi.wk.login.ord) %>%
                separate(epi.wk.login.ord, into = c("yr", "wk"), sep = "-") %>%
                arrange(yr, wk) %>%
                distinct(epi.wk.login)

# Define the colour for the grey box

grey_trans <- rgb(179, 179, 179, max = 255, alpha = 10)

# identify most recent epiweek

end.epi.wk <- paste(epiyear(max(meta.df$Login.Date)), sprintf( "%02d", epiweek(max(meta.df$Login.Date))), sep = "-")
...
subset_completeness <- subset(meta.df, genome_completeness >= 0.9)

# get the highest weekly count for the scale
weekly_max <- subset_completeness %>% group_by(epi.wk.login) %>% summarise(counts = n()) %>%
  filter(counts == max(counts))

ggplot(data=subset_completeness) +

                       #& epi.wk.login < '2021-01')) +

     geom_rect(aes(xmin = which(epiwk.list==end.epi.wk)-2.5,

                  xmax = which(epiwk.list==end.epi.wk)+0.5,
              ...
              fill=grey_trans) + annotate("text", x = which(epiwk.list==end.epi.wk)-2.45, y = Inf,
             ...
             size = 5.5, vjust = 1.2, hjust = -.03, color = "black") +

   # use labs tag to place the comment for sequencing over the rectangle
    labs(tag = "Samples recieved\nduring this time\nperiod may not\nyet be sequenced") +
    theme(plot.tag.position = c(0.96, 0.9)) +
        #annotate("text", x = which(epiwk.list==end.epi.wk)-9, y = Inf,

             #label = "Samples recieved during this time \nperiod may not yet be sequenced",
  ...
  # specify limits in y scale continuous instad of ylim to get better intervals
    scale_y_continuous("No. samples", breaks = seq(0, round(max(weekly_max$counts), -2) + 2000, 100),
                       limits = c(0, round(max(weekly_max$counts), -2) + 200))
  
```
  
```{r, echo=F, message=F, warning=F, fig.width=13, include=FALSE}
## THIS CODE CORERSPONDS TO THE SEQUENCING SUMMARY FOIR 2020. It has been parked for now. 
### SARAH SECTION 2 - Weekly Sequencing Summary ---- ###
...
meta.df$genome_completeness <- as.numeric(meta.df$genome_completeness)
  ...
  ggplot(data=subset(meta.df, genome_completeness >= 0.9 & epi.wk.login < '2021-01')) +
    ...
    scale_y_continuous("No. samples", expand = c(0,10)) 
```
\
\
```{r, include=FALSE}
...
\
Pango lineages from sequenced samples from January 1, 2021 to the current report date. 
\
...
# placeholder
```
## Sequence Categories by Epidemiological week, 2021
\
An overview of the Pango lineage categories for this figure can be found in the [Technical Notes](#technical-notes) section. 
\
```{r, include=FALSE}
cat_over_time <- meta.df %>% group_by(epi.wk.login, lineage.all.cat) %>% dplyr::summarise(samples = n())
cat_time_plot <- ggplot(cat_over_time, aes(x = epi.wk.login, y = samples, fill = lineage.all.cat)) +
  ...
  theme(axis.text.x = element_text(angle = 90, size = 16),
        ...
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        legend.title = element_text(size = 16),
  legend.text = element_text(size = 13)) + labs(fill = "Category") + xlab("Epidemiological Week") + ylab("Sample Counts") + ylim(c(0, max(cat_over_time$samples) + 1000)) +
  # specify y axis line breaks
  scale_y_continuous(breaks = seq(0, plyr::round_any(max(cat_over_time$samples), 100) + 1000,
                                  100))

voc_sep <- "B.1.1.7, B.1.351, P.1"
voi_sep <- "P.2, P.3, B.1.525, R.1, B.1.1.318, B.1.427, B.1.429, B.1.526, B.1.617, B.1.617.1, B.1.617.2, B.1.617.3, B.1.618, B.1.2 with S:E484K"
...
```
```{r, fig.height=12, fig.width=13, echo=FALSE, include=F}
cat_time_plot
```
\
\
\
```{r, fig.width = 18, fig.height = 10, echo=F, warning=F, message=F}
# Order the epi week for the rectangle

epiwk.list <- meta.df.sub %>%
...
end.epi.wk <- paste(epiyear(max(meta.df.sub$Login.Date)), sprintf( "%02d", epiweek(max(meta.df.sub$Login.Date))), sep = "-")


# ---- Plot VOC/VOIs by Epi week ----

grouped_by_epi <- meta.df.sub %>% 

    group_by(epi.wk.login, lineage.all.cat) %>%

    mutate(lineage_id = ifelse(is.na(lineage.all.cat), "None", ifelse(lineage.all.cat == "non-VoC/non-VoI", "Other", lineage.all.cat))) %>% group_by(epi.wk.login, lineage_id) %>%

    dplyr::summarise(n = n())

grouped_by_epi$lineage_id <- factor(grouped_by_epi$lineage_id, levels=c("VoC", "VoI", "Other",
                                                                        "None"))


ggplot(grouped_by_epi, aes(x = epi.wk.login, y = n, fill = lineage_id)) +
  geom_rect(aes(xmin = which(epiwk.list==end.epi.wk)-2.5,
  ...
  geom_bar(stat = "identity") +
    ...
    scale_y_continuous("No. samples", expand = c(0,10), limits = c(0, 1000)) + # Need to create limits automatically
...
A lis tof the current Pango lineages that are classified as a VoC by PHO can be found in the  [Technical Notes](#technical-notes) section. 
\
...
max_date_each_voc <- rolling_cases %>% group_by(lineage) %>% filter(date == max(date))
...
start.s5 <- (max(rolling_cases$date) - 21)

  # end point = most recent login date

end.s5 <- max(rolling_cases$date)
...
plot_voc <- ggplot(rolling_cases, aes(x = date, y = cumcases, color = lin_fac)) + geom_point(size = 3) +
  ...
  theme_bw() + theme(axis.text.x = element_text(angle = 0, size = 14),
   ...
   geom_line(color='black',data = rolling_cases, aes(x = date, y = cumcases)) +
     geom_text(data=rect.s5, aes(x = xmin, y = ymin, label = "Samples recieved \nduring this time period\nmay not yet be\nsequenced"), size = 5, vjust = -1, hjust = -0.05, color = "black") +
...
\
\
```{r, warning=FALSE, fig.height=20, fig.width=14, echo=FALSE}
plot_voc
```
\
\
...
## Relative frequency by Public Health Unit of VoCs
...
for (i in voc) {
  health_region_over_time <- subset(meta.df.sub, lineage_all == i) %>%
...
\
A list of the current Pango lineages that are classified as VoIs by PHO can be found in the  [Technical Notes](#technical-notes) section. 

```{r, include = FALSE, echo=FALSE}

voi_list <- c("P.2", "P.3", "B.1.525", "R.1", "B.1.1.318", "B.1.427", "B.1.429", "B.1.526",
              "B.1.617", "B.1.617.1", "B.1.618", "B.1.617.2", "B.1.617.3")

subset_voi_date <- subset(qc_data_recent, lineage_all %in% voi_list & lineage_all != "None") %>%
  arrange(date) %>%
  group_by(lineage_all, date) %>% 
  dplyr::summarise(lin_counts = n())

rolling_cases_voi <- subset_voi_date %>% group_by(lineage_all, .drop = FALSE) %>%
  mutate(cumcases = cumsum(lin_counts))

setnames(rolling_cases_voi, old = c('lineage_all'), new = c('lineage'))

max_date_each_voi <- rolling_cases_voi %>% group_by(lineage) %>% filter(date == max(date))
...
# set this specific order to place the blank cell next to b117 and have other 
# two vocs on separate line
rolling_cases_voi$lin_fac <- factor(rolling_cases_voi$lineage, levels=c("P.2", "P.3", "B.1.525", "R.1", "B.1.1.318", "B.1.427", "B.1.429", "B.1.526",
              "B.1.617", "B.1.617.1", "B.1.618", "B.1.617.2", "B.1.617.3"))

plot_voi <- ggplot(rolling_cases_voi, aes(x = date, y = cumcases, color = lin_fac)) + geom_point(size = 3) + # + scale_color_manual(values = c(VOC.pal)) +
  # facet wrap with as fable allows for blank cell in col 2 cell 1
  # https://stackoverflow.com/questions/56447366/ordering-placement-of-empty-facet-when-using-facet-wrap
  facet_wrap(~lin_fac, ncol=1, scales = "free_y", as.table = F) +
  geom_rect(data=rect.s5, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax),
  ...
  theme_bw() + theme(axis.text.x = element_text(angle = 0, size = 14),
          ...
          legend.position = "none",
                       ...
                       panel.background = element_blank(), axis.line = element_line(colour = "black"),
          strip.background =element_rect(fill="white")) +
   geom_line(color='black',data = rolling_cases_voi, aes(x = date, y = cumcases)) +
     geom_text(data=rect.s5, aes(x = xmin, y = ymin, label = "Samples recieved \nduring this time period\nmay not yet be\nsequenced"), size = 5, vjust = -1, hjust = -0.05, color = "black") +
    # ggtitle(paste("Watchlist lineages (VoC) as of ", format(last(rolling_cases$date), "%b-%d-%Y"), #    sep="")) +
    xlab("Login Date") +
    ylab("Cumulative number of sequences") + scale_x_date(date_labels = "%b-%Y") +
...
max_counts_voi <- rolling_cases_voi %>% group_by(lin_fac) %>% filter(cumcases == max(cumcases))
max_counts_voi <- subset(max_counts_voi, select = c(lineage, cumcases)) %>% arrange(-cumcases)
colnames(max_counts_voi) <- c("lineage", "Sequence number")
...
cols.voi <- c("#31B98F", "639B8B","#B7B7B7",  "#CF913C", "#819EE9", "#741F39", "#D49998","#1F343C", "#C144BC")

voi_groupings <- meta.df.sub[meta.df.sub$lineage_all %in% voi_list,] 

voi_groupings <- voi_groupings %>%

  group_by(epi.wk.login, lineage_all) %>%
...
ggplot(voi_groupings, aes(x = epi.wk.login, y = n, fill = lineage_all)) +
...
colnames(all_muts_frame) <- c("mutation", "VoC_samples", "Percent_VoC", "Gene")

all_muts_frame <- all_muts_frame %>% arrange(-Percent_VoC)
...
colnames(all_muts_frame_no_voc) <- c("mutation", "non_VoC_samples", "Percent_non_VoC", "Gene")

all_muts_frame_no_voc <- all_muts_frame_no_voc %>% arrange(-Percent_non_VoC)

```

```{r, results='asis', echo=FALSE}
merge_mutations <- merge(all_muts_frame, all_muts_frame_no_voc, by.x = "mutation",
                         by.y = "mutation", all=T) %>% subset(select=c(-Gene.x)) %>% arrange(-Percent_VoC)
...
table_mutations_non_voc <- subset(max_date_each_lin_non_voc, select = c(mutation, lineage, cumcases)) %>%
  arrange(-cumcases)
setnames(table_mutations_non_voc, "cumcases", "samples")
