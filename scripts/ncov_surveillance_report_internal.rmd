...
# ---->>>> List of VOCs <<<<----
voc.list <- c("B.1.1.7", "B.1.351", "P.1", "B.1.617", "B.1.617.1", "B.1.617.2", "B.1.617.3")  # Add B.1.1.7 with E484K ?
...
# Use 9 palettes from the colorspace package, one for each parent lineage  #### Need to think aobut this in case more parent lineages than 9
...
meta.df.all.wks %>% ggplot() +
                       geom_rect(aes(xmin = which(epiwk.list.all==end.epi.wk)-8.5,
...
lin.changes.since.last <- meta.df.2 %>%
                           ...
                           # mutate(n.perc.chng = paste0(New, " (", perc.chng, ")", sep = "")) %>%
                            ...
                            dplyr::select(Category, Lineage = lineage.all, tot.lin, rel.freq,
                                   New, perc.chng)



# datatable(as.data.frame(lineage_info), #extensions = 'Buttons',
...
sketch = htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(colspan = 2, 'Total Genomes', style = "border-right: solid 6px #595959;
                                                border-bottom: solid 1px white;"),
      th(colspan = 2, 'Change in Genomes*', style = "border-left: solid 6px #595959;
                                                    border-bottom: solid 1px white;")
    ),
...
#reactable(lin.changes.since.last)
# 
# reactable(
#   lin.changes.since.last,
#   searchable = TRUE,
#   showSortable = TRUE,
#  # striped = TRUE,
#   highlight = TRUE,
...
# 
#   columns = list(
#     Category = colDef(name = "Category", align = "center", minWidth = 125),
#     lineage.all = colDef(name = "Lineage", minWidth = 125),
#     tot.lin = colDef(name = "N", minWidth = 100),
#     rel.freq = colDef(name = "%", align = "right", minWidth = 100),
#     New = colDef(name = "N", minWidth = 100),
#     perc.chng = colDef(name = "%", align = "right", minWidth = 100) 
#   ),
#  #header = list(lineage.all),
#  defaultColGroup = colGroup(headerStyle = list(borderTop = "1.5px solid #595959")),
#  defaultColDef = colDef(headerStyle = list(borderTop = "1.5px solid #595959", 
#                                            borderBottom = "1.5px solid #595959")),
...
#     ),  
#  # cellPadding = "8px 12px"
#   # defaultColDef = colDef(headerStyle = list(background = "#595959")
#   # ),  style = list(borderTop = "1px solid #eee"),
#  
#    columnGroups = list(
#     colGroup(name = "Total Genomes", columns = c("tot.lin", "rel.freq")),
#     colGroup(name = "", columns = c("Category", "lineage.all")),
#     colGroup(name = "Change in Genomes", columns = c("New", "perc.chng")))) # headerStyle = list(background = "#595959"
# #) 
        ...
        dplyr::summarise(n = n()) %>%
          ggplot(aes(x = epi.wk.login, y = n, fill = lineage.all.cat)) +
             geom_rect(aes(xmin = which(epiwk.list.all==end.epi.wk)-8.5,
                                 ...
                                 ymin=-Inf, ymax=Inf),
                                 fill="grey90") + 
                                 annotate("text", x = which(epiwk.list.all==end.epi.wk)-8.45, y = Inf, 
                                                  label = "Subject to \nsequencing lag",
                                                  size = 5, vjust = 1.2, hjust = -.03, color = "black") +
...
cols.voi <- c("#6B013E", "#CD398D", "#EAA1CB","#F4C3E1","grey50", "grey80", 
              "#C7DDE3", "#A5C8D2", "#629FB0", "#325F6C")
...
# lineages <- c("B.1.1.7", "B.1.351", "P.1", "P.2", "P.3", "B.1.525", "R.1", "B.1.1.318", "B.1.427", "B.1.429", "B.1.526", "B.1.526.1", "B.1.617", "B.1.617.1", "B.1.617.2", "B.1.617.3")
# 
...
# subset_other_lineages <- subset(meta.df.2021, ! lineage.all %in% lineages) %>%
#   arrange(date) %>%
...
#   sample_no_pass <- as.data.frame(tr.df.labs[!tr.df.labs$label %in% meta.df.2$WGS_Id,])
...
#   meta.df.2$date_cat <- ifelse(meta.df.2$date %within% date_cat, "Latest Week", "All Past Weeks")
#   qc50_keep <- subset(meta.df.2, select = c(WGS_Id, date, date_cat))
...
## Mutations of Concern with VoC/VoI designations

```{r, echo = F, message=F, warning=F, fig.height=12, fig.width=13, include = F}
# test a frame with 484 counts for both VOI and VOCS (INCOMPLETE IN PROGRESS)

only_484_voc <- merged_voc_ontario[grepl("S:E484", merged_voc_ontario$aaSubstitutions),] %>%
  group_by(lineage_all) %>% summarise(Spike_484_counts = n())  %>% rename(voc.list = lineage_all)

voc_info <- merge(VOC.df, only_484_voc, by = "voc.list", all = T) %>% rename(lineage = voc.list) %>%
  mutate(Category = "VoC")

only_484_voi <- merged_non_voc_ontario[grepl("S:E484", merged_non_voc_ontario$aaSubstitutions),] %>%
  subset(lineage.all %in% voi.list) %>%
  group_by(lineage_all) %>% summarise(Spike_484_counts = n()) %>% mutate(Category = "VoI") %>% rename(voi.list = lineage_all)

voi_info <- merge(VOI.df, only_484_voi, by = "voi.list", all = T) %>% rename(lineage = voi.list) %>%
  mutate(Category = "VoI")

all_with_484_counts <- rbind(voc_info, voi_info) %>% mutate(Spike_484 = ifelse(!is.na(Spike_484_counts), "X",""))

datatable(as.data.frame(all_with_484_counts[,c(1, 3, 4, 2)]), 
          options = list(pageLength = 20, 
                   # columnDefs = list(list(className = 'dt-left', targets = 1:3),
                   #                   list(className = 'dt-center', targets = 4)),
                   autowidth = TRUE,initComplete = JS(
...
replace_lin_grouped <- replace_lineages %>% group_by(Login_Date, reassign) %>% dplyr::summarise(counts = n()) %>%
  group_by(Login_Date, reassign) %>% dplyr::summarise(cumcounts = cumsum(counts))

dates_reassign <- replace_lineages %>% group_by(reassign) %>% dplyr::mutate(counts = n()) %>% filter(Login_Date == min(Login_Date) | Login_Date == max(Login_Date)) %>% 
