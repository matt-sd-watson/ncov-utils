...
parser = argparse.ArgumentParser(description='Parse an indexed bam file for mutation frequencies at set coordinates')
parser.add_argument('--mutation_ids', '-m', type=str, help='CSV file with mutations of interest in type variants format', required=True)
parser.add_argument('--input_bam', '-i', type=str, help='Input ncov bam file with index file in the same directory ', required=True)
